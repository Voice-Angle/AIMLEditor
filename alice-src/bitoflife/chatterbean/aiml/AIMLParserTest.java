...
public class AIMLParserTest extends TestCase {
	...
	public void testParseThatTopicAIML() throws Exception {
		...
		parser.parse(mock, new FileInputStream(
				"Bots/mydomain/dev-examples.aiml"));
		java.lang.System.out.println(mock.categories.size());
		assertEquals(4, mock.categories.size());

		Category expected = new Category(new Pattern(" YES "), new That(
				" DO YOU LIKE CHEESE "), new Template("Good for you.",
				new Think(new Set("topic", "*"))));
	...
	public void testParseAgainAIML() throws Exception {
		...
		category = new Category(new Pattern(" _ AGAIN "), new Template(
				"Once more? ", new Srai(1)));
		Category actual = categories.get(category.getPattern());
		assertNotNull(actual);
		assertEquals(category, actual);

		category = new Category(new Pattern(" _ ALICE "), new Template(
				new Srai(1)));
		assertEquals(category, categories.get(category.getPattern()));

		category = new Category(new Pattern(" YOU MAY * "), new Template(
				new Srai(1)));
		assertEquals(category, categories.get(category.getPattern()));

		category = new Category(new Pattern(" SAY * "), new Template("\"",
				new Star(1), "\"."));
		assertEquals(category, categories.get(category.getPattern()));
	}
}
